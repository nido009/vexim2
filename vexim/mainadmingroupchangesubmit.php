...
/*  if (!isset($_POST['is_public'])) {
  ...
  if (preg_match("/['@%!\/\|\" ']/",$_POST['localpart'])) {
    header("Location: mainadmingroupchange.php?group_id={$_POST['group_id']}&badname={$_POST['localpart']}");
    die;
  }
  ...
  $success = $sth->execute(array(':localpart'=>$_POST['localpart'], ':enabled'=>$_POST['enabled'],
      ':is_public'=>$_POST['is_public'], ':group_id'=>$_POST['group_id'], ':domain_id'=>$_SESSION['local_domain_id']));
  if ($success) { 
