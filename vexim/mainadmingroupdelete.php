<?php
  ...
  if(array_key_exists('confirm', $_GET)) {
    if ($_GET['confirm'] == '1') {
	  ...
	  if (!$sth->rowCount()) {
  	    header ("Location: mainadmingroup.php?group_faildeleted={$_GET['localpart']}");
	    die();  
	  }
      ...
      if ($success) {
        ...
        $sth = $dbh->prepare($query);
        $success = $sth->execute(array(':group_id'=>$_GET['group_id'], ':domain_id'=>$_SESSION['local_domain_id']));
        if ($success) {
          header ("Location: mainadmingroup.php?group_deleted={$_GET['localpart']}");
          die;
        } else {
          header ("Location: mainadmingroup.php?group_faildeleted={$_GET['localpart']}");
          die;
...
<html>
  <head>
    ...
    <div id="Content">
      <form name="groupdelete" method="get" action="mainadmingroupdelete.php">
        <table align="center">
          <tr>
            <td colspan="2">
