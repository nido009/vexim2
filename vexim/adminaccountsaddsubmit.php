<?php
  ...
  if ($success) {
    $domrow = $sth->fetch();
    if (!$domrow['allowed']) {
        header ("Location: adminaccounts.php?maxaccounts=true");
        die();
    }
  }
  ...
  if($row['quotas'] != "0") {
    if (($_POST['quota'] > $row['quotas']) || ($_POST['quota'] == "0")) { 
      header ("Location: adminaccounts.php?quotahigh={$row['quotas']}");
      die; 
    }
  }
  ...
  check_user_exists(
    $dbh,$_POST['localpart'],$_SESSION['domain_id'],'adminaccounts.php'
  );

  if (preg_match("/^\s*$/",$_POST['realname'])) {
    header('Location: adminaccounts.php?blankname=yes');
    die;
  }

  if (preg_match("/['@%!\/\| ']/",$_POST['localpart'])
    || preg_match("/^\s*$/",$_POST['localpart'])) {
    header("Location: adminaccounts.php?badname={$_POST['localpart']}");
    die;
  }
  ...
  if (validate_password($_POST['clear'], $_POST['vclear'])) {
    ...
    if ($success) {
      header ("Location: adminaccounts.php?added={$_POST['localpart']}");
      ...
      header ("Location: adminaccounts.php?failadded={$_POST['localpart']}");
      die;
    }
  } else {
    header ("Location: adminaccounts.php?badpass={$_POST['localpart']}");
    die;
  }
?>
