<?php
  ...
  if ($_POST['localpart']==''){
    header("Location: mainadminalias.php?badname={$_POST['localpart']}");
    die;
  }

  # check_user_exists() will die if a user account already exists with the same localpart and domain id
  check_user_exists(
    $dbh,$_POST['localpart'],$_SESSION['local_domain_id'],'mainadminalias.php'
  );
  ...
  if (alias_validate_password($_POST['clear'], $_POST['vclear'])) {
    ...
    if ($success) {
      header ("Location: mainadminalias.php?added={$_POST['localpart']}");
    } else {
      header ("Location: mainadminalias.php?failadded={$_POST['localpart']}");
    }
  } else {
    header ("Location: mainadminalias.php?badaliaspass={$_POST['localpart']}");
  } 
?>
