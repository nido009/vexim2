...
$sth->execute();
if (!$sth->rowCount()) {
  header ("Location: adminaccounts.php?faildeleted={$_GET['localpart']}");
  die();  
}
...
if ($_GET['confirm'] == '1') {
	...
	if (!$sth->rowCount()) {
		...
		$success = $sth->execute();
		if ($success) {
			header ("Location: adminaccounts.php?deleted={$_GET['localpart']}");
			die;                                                      
		} else {
			header ("Location: adminaccounts.php?faildeleted={$_GET['localpart']}");
			die;                                                      
...
<html>
  ...
  <body>
    ...
    <div id="menu">
      <a href="adminaccountsadd.php">Administrator hinzuf&uuml;gen</a><br>
      <a href="admin.php"><?php echo _('Main Menu'); ?></a><br>
      <br><a href="logout.php"><?php echo _('Logout'); ?></a><br>
    </div>
    <div id="Content">
      <form name="admindelete" method="get" action="adminaccountsdelete.php">
        <table align="center">
          <tr>
            <td colspan="2">
