...
if ($_GET['confirm'] == '1') {
  ...
  $query = "SELECT COUNT(user_id) AS count FROM users 
    ...
    AND user_id!=:user_id";
  $sth = $dbh->prepare($query);
  $sth->execute(array(':domain_id'=>$_SESSION['local_domain_id'], ':user_id'=>$_GET['user_id']));
  $row = $sth->fetch();
  if ($row['count'] == "0") {
    header ("Location: mainadminuser.php?lastadmin={$_GET['localpart']}");
    die;
  }  
...
<html>
  ...
  <body>
    ...
    <div id="menu">
      <a href="mainadminuseradd.php"><?php echo _('Add User'); ?></a><br>
      <a href="mainadmin.php"><?php echo _('Main Menu'); ?></a><br>
      <br><a href="logout.php"><?php echo _('Logout'); ?></a><br>
    </div>
    <div id="Content">
      <form name="userdelete" method="get" action="mainadminuserdelete.php">
        <table align="center">
          <tr>
            <td colspan="2">
