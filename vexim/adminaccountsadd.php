...
<html>
  ...
  <body onLoad="document.adminadd.realname.focus()">
    ...
    <div id="menu" style="width: 170px;">
      <a href="adminaccounts.php"><?php echo _('Administratoren verwalten'); ?></a><br>
      <a href="admin.php"><?php echo _('Main Menu'); ?></a><br>
      <br><a href="logout.php"><?php echo _('Logout'); ?></a><br>
    </div>
    <div id="forms" style="width: 700px;">
    <form name="adminadd" method="post" action="adminaccountsaddsubmit.php">
      <table align="center">
        <tr>
        ...
        <tr>
          ...
          <td colspan="2" style="float: left; width: 500px;">
			...
			<select name="domain" class="domain">
				<option value="<?php echo $_SESSION['domain_id'] ?>"><?php echo $_SESSION['domain'] ?></option>
<?php
$SQL = "SELECT * FROM domains WHERE domain != 'admin' AND domain != 'dummydomain'";
$sth = $dbh->prepare($SQL);
$sth->execute();
while ($row2 = $sth->fetch()) {
	if ($row2['domain_id'] != $_SESSION['domain_id']) {
		echo "<option value=\"".$row2['domain_id']."\">".$row2['domain']."</option>";
	}
}
?>
			</select>
      ...
      <?php } ?>
        <tr>
		  ...
		  <select name="admin">
			...
			<option value="2">Hauptadministrator</option>
		  </select>
		  </td>
        </tr>
        <?php if ($row['pipe'] == "1") { ?>
