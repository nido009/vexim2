...
if (!$sth->rowCount()) {
  header ("Location: siteadmin.php?faildeleted={$_GET['localpart']}");
  die();  
}
...
if ($_GET['confirm'] == '1') {
  ...
  $sth = $dbh->prepare($query);
  $success = $sth->execute(array(':user_id'=>$_GET['user_id']));
  if ($success) {
    header ("Location: siteadmin.php?deleted={$_GET['localpart']}");
  } else {
    header ("Location: siteadmin.php?faildeleted={$_GET['localpart']}");
  }
...
<html>
  ...
  <body>
    ...
    <div id="Content">
      <form name="userdelete" method="get" action="siteadmindelete.php">
        <table align="center">
          <tr>
            <td colspan="2">
              <?php printf (_('Please confirm deleting user %s'),
                $row2['localpart']);
              ?>:
            </td>
          </tr>
          <tr>
            <td>
              ...
              <b><?php printf (_('Do Not Delete %s'),
                $row2['localpart']);
              ?></b>
            </td>
          </tr>
