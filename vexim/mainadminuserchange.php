<?php
  ...
  $sth->execute(array(':user_id'=>$_GET['user_id'], ':domain_id'=>$_SESSION['local_domain_id']));
  ...
  $domsth->execute(array(':domain_id'=>$_SESSION['local_domain_id']));
  if ($domsth->rowCount()) {
    $domrow = $sth->fetch();
  }
...
<html>
  ...
  <?php include dirname(__FILE__) . '/config/header.php'; ?>
    ...
    <table align="center">
      <form name="userchange" method="post" action="mainadminuserchangesubmit.php">
        <tr>
          ...
          <td>
            ...
            <select name="forwardmenu">
              ...
              <?php
				...
				$sthuserlist->execute(array(':domain_id'=>$_SESSION['local_domain_id']));
        ...
        <tr>
          ...
          <?php
			...
			$query = "SELECT user_id,localpart,domain,realname FROM users,domains
				WHERE smtp=:smtp'{$row['localpart']}@{$_SESSION['local_domain']}'
				AND users.domain_id=domains.domain_id ORDER BY realname";
            $sth = $dbh->prepare($query);
			$sth->execute(array(':smtp'=>$row['localpart'].'@'.$_SESSION['local_domain']));
            if ($sth->rowCount()) {
              while ($row = $sth->fetch()) {
				$d = $_SESSION['local_domain'];
                if (($row['domain'] == $d)
                  && ($row['localpart'] != "*")) {
                  print '<a href="mainadminaliaschange.php?user_id='
                    . $row['user_id']
                    . '">'
                    . $row['localpart']. '@' . $row['domain']
                    . '</a>';
                } else if (($row['domain'] == $d)
                  && ($row['localpart'] == "*")) {
                  print '<a href="mainadmincatchall.php?user_id='
                    . $row['user_id']
                    . '">'
                    . $row['localpart'] . '@' . $row['domain']
                    . '</a>';
    ...
    <table align="center">
      <form name="blocklist" method="post" action="mainadminuserblocksubmit.php">
        <tr>
          <td colspan="2">
            <?php
    ...
    <table align="center">
      ...
      ?>
            <tr>
              <td>
                <a href="mainadminuserblocksubmit.php?action=delete&user_id=<?php 
					print $_GET['user_id']
					. '&block_id='
					. $blockrow['block_id']
					.'&localpart='
