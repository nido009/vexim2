<?php
  ...
  check_user_exists(
    $dbh,$_POST['localpart'],$_SESSION['local_domain_id'],'mainadminfail.php'
  );

  if (preg_match("/['@%!\/\| ']/",$_POST['localpart'])) {
    header("Location: mainadminfail.php?badname={$_POST['localpart']}");
    die;
  }
  ...
  if ($success) {
    header ("Location: mainadminfail.php?added={$_POST['localpart']}");
  } else {
    header ("Location: mainadminfail.php?failadded={$_POST['localpart']}");
  }
?>
